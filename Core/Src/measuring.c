...
bool DAC_active = false;				 ///< continue incrementing DAC sweep in interrupt

uint32_t ADC_samples[NUM_CHANNEL * ADC_NUMS];
uint16_t batt_sample;
...
/*
 * configure timers (all interrupts are used)
 */
void MEAS_timer_init(void)
{
	// DOPP timer (interrupt used)
	...
	// TODO reenable interrupt/etc since currently we just use the other timer
...
/*
 * initialize DAC to work on GPIO PA5 w/ DAC_OUT2
 */
void DAC_init(void)
{
...
/*
 * called periodically during sweep
 */
...
/*
* TIM1 (FMCW) interrupt (unused)
*/
void TIM1_IRQHandler(void)
{
	TIM1->SR &= ~TIM_SR_UIF; // Clear pending interrupt flag
}

/*
* TIM2 (DOPP) interrupt (unused)
*/
...
/*
* TIM5 (FMCW sweep) interrupt (used to trigger DAC increment)
*/
 ...
 *
 * reads in values from ADCs to buffer to be processed by calculation
 * functions later
 * stops reading when buffer is filled
 *****************************************************************************/
void ADC_IRQHandler(void)
{
	// TODO clean up measurements here since we use the same ADCs for both FMCW and DOPP (can we share the measurement buffer?)
	// DOPP measurements read from PC1/PC3 (ADC1/2)
	if (DOPP_active)
	{ // read from ADC1/3
		if (ADC1->SR & ADC_SR_EOC)
		{												 // Check if ADC1 end of conversion
			ADC_samples[ADC1_sample_count++] = ADC1->DR; // Read input channel 1 only

			if (ADC1_sample_count >= 2 * ADC_NUMS)
			{								// Buffer full
		...
		// TODO this block is unused, no? we shouldn't be using ADC3.
		if (ADC3->SR & ADC_SR_EOC)
		{												 // Check if ADC3 end of conversion
			ADC_samples[ADC3_sample_count++] = ADC3->DR; // Read input channel 1 only

			if (ADC3_sample_count >= 3 * ADC_NUMS)
			{								// Buffer full
				TIM2->CR1 &= ~TIM_CR1_CEN;	// Disable timer
	...
	if (FMCW_active)
	{
		...
		{														  // Check if ADC1 end of conversion
			...
			if (ADC_FMCW_sample_count >= FMCW_ADC_SAMPLES)
			{								// Buffer full
	...
	{ // read from ADC2
		...
		{														  // Check if ADC2 end of conversion
...
// DMA2 stream 4 are what we use, i thought. TODO double check and see if these are used
/** ***************************************************************************
...
void DMA2_Stream1_IRQHandler(void)
{
	...
	{											 // Stream1 transfer compl. interrupt f.
...
/*
* clear sample buffer arrays w/ 0
*/
...
void ADC_DOPP_scan_init(void)
{
	...
	// PC3 -> ADC2 -> DMA
	// in dual mode, this uses a single DMA channel (DMA2str4)
...
{
	// NOTE this is just using dual mode. we only care about PC4 -> ADC1 though
